...
int main(int argc, char* argv[]) {
    ...
    unsigned long read_lens[] = {1,   2,   4,   8,    16,   32,   64,
                                 128, 256, 512, 1024, 2048, 4096, 8192};
    double read_times[14];
    ...
    for (i = 0; i < N_ELEMS(read_lens); i++) {
        ...
        for (j = 0, read_times[i] = 0.0; j < 1000; j++) {
        ...
        printf("Transfer speed (%lu word(s)) = %0.6f MB/s\n", read_lens[i],
               (((((double) read_lens[i] * 2.0) + 7.0) * 1000.0) / 1045876.0) /
                   read_times[i]);
    }
