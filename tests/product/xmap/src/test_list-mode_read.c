...
int main(int argc, char* argv[]) {
    ...
    while (difftime(time(NULL), start) < n_secs) {
        ...
        if (!first_buffer) {
            status = xiaBoardOperation(0, "buffer_switch", &ignore);

            if (status != XIA_SUCCESS) {
                xiaStopRun(-1);
                xiaExit();

                fprintf(stderr, "Error switching buffers.\n");
                exit(1);
            }
        } else {
            fprintf(stdout, "Skipping buffer switch for first buffer.\n");
            first_buffer = FALSE_;
        }
