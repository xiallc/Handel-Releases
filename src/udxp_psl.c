...
PSL_STATIC int pslBuildDSPList(int detChan);

PSL_STATIC int pslGetParamIdx(const char *name, unsigned int *idx);
...
static SpecialRun traceTypes[] = {
  ...
  {"debug",                          NULL},   // This needs to be the last value
};
...
/* XXX This hack to use globals for the DSP parameter list will fail if
 * multiple boards are used at once, unless they have the exact same DSP
 * parameters. To fix this, pslBuildDSPList() needs to be changed into a
 * board operation.
 */
static unsigned int nParams = 0;
static char **paramNames = NULL;
...
/** @brief Required by Handel, but unimplemented for this product
 *
 */
...
/** @brief Retrieves the specified acquisition value from either the cache
                                       ...
                                       XiaDefaults *defs)
{
  ...
  sprintf(info_string, "acquisition value '%s' = %0.3f", name, *((double*)value));
                           ...
                           Module *m)
{
  ...
  sprintf(info_string, "Started a run w/ id = %u",
          (unsigned int)(receive[5] + (receive[6] << 8)));
...
PSL_STATIC int pslStopRun(int detChan, Module *m)
{
  ...
  /* Close the port before we attempt
   * to do a normal IO operation on
   * it.
   */
  /*CLOSE_PORT;*/
  ...
  if (statusX != DXP_SUCCESS) {
    status = XIA_XERXES;
    sprintf(info_string,
            "Error stopping run on detChan %d",
            detChan);
...
PSL_STATIC int pslDoTrace(int detChan, short type, double *info, XiaDefaults *defs)
{
  ...
  */
  ...
  if (IS_USB && dxp_has_direct_trace_readout(detChan)) {
    DEFINE_CMD(CMD_READ_ADC_TRACE, 3, 2);
...
/** @brief A much abused routine that returns both the default alias and
 *  the default values
 *
 */
...
PSL_STATIC int pslGetParameter(int detChan, const char *name, unsigned short *value)
{
  ...
  byte_t cmd       = CMD_READ_DSP_PARAMETER;

  byte_t send[2];
  byte_t receive[3 + RECV_BASE];


  status = pslGetParamIdx(name, &idx);

  if (status != XIA_SUCCESS) {
    sprintf(info_string,
            "Error getting index of %s",
            name);
  ...
  if (statusX != DXP_SUCCESS) {
    status = XIA_XERXES;
    sprintf(info_string,
            "Error reading out DSP parameter: %s",
            name);
    pslLogError("pslGetParameter", info_string, status);
    return status;
  }

  *value = (unsigned short)(receive[5] | (unsigned short)receive[6] << 8);

  sprintf(info_string, "%s = %u", name, *value);
  pslLogDebug("pslGetParameter", info_string);
...
PSL_STATIC int pslSetParameter(int detChan, const char *name, unsigned short value)
{
  ...
  byte_t cmd       = CMD_WRITE_DSP_PARAMETER;
  ...
  send[0] = (byte_t)1;
  send[1] = (byte_t)idx;
  send[2] = LO_BYTE(value);
  send[3] = HI_BYTE(value);
                            ...
                            int detector_chan, Module *m, int modChan)
{
  ...
  sprintf(info_string, "Preparing to update DSP list for detChan %d",
          detChan);
  pslLogDebug("pslUserSetup", info_string);

  /* This routine should be removed from pslUserSetup() since it
   * will be controlled through pslBoardOperation().
   */
  status = pslBuildDSPList(detChan);

  if (status != XIA_SUCCESS) {
    pslLogError("pslUserSetup",
                "Error building peaking time information",
                status);
    return status;
  }
...
/** @brief Return the number of required defaults in the acquisition values
...
PSL_STATIC int pslGetNumParams(int detChan, unsigned short *numParams)
{
  ...
  sprintf(info_string, "nParams = %u", nParams);
  pslLogDebug("pslGetNumParams", info_string);

  *numParams = (unsigned short)nParams;
...
PSL_STATIC int pslGetParamData(int detChan, char *name, void *value)
{
  ...
  unsigned short nFullReads = (unsigned short)floor((double)(nParams / maxWordsPerTransfer));


  /* "names" is not implemented yet */
  if (STREQ(name, "names")) {
    status = XIA_NOSUPPORT_VALUE;
    pslLogError("pslGetParamData",
                "'names' is currently not a supported data type",
                status);
    return status;
  }
  ...
  if (STREQ(name, "access")) {
    ...
    for (addr = 0x0000;
         addr < (nFullReads * maxWordsPerTransfer);
      ...
      if (statusX != DXP_SUCCESS) {
        status = XIA_XERXES;
        pslLogError("pslGetParamData", "Error reading DSP data memory", status);
        return status;
      }
    ...
    finalR = (byte_t *)utils->funcs->dxp_md_alloc(lenR * sizeof(byte_t));
...
PSL_STATIC int pslGetMCAData(int detChan, void *value, XiaDefaults *defs)
{
   ...
   * The trick here is that each data point may
   * be up to 3 bytes long as specified by the
   * user.
   */
  for (i = 0; i < numMCAChans; i++) {
    data[i] = 0x00000000;
    for (j = 0; j < (unsigned int)bytesPerBin; j++) {
      data[i] |= (unsigned long)((
                                   (unsigned long)receive[(i * (unsigned int)bytesPerBin) + j + RECV_BASE] & 0xFF)
                                 << (j * 8));
    }
  }
...
PSL_STATIC int pslGetBaseline(int detChan, void *value, XiaDefaults *defs)
{
  ...
  for (i = 0; i < BASELINE_LEN; i++) {
    data[i] = (unsigned long)(receive[(i * 2) + 5] |
                              ((unsigned long)receive[((i * 2) + 1) + 5] & 0xFF) << 8);
  }
...
/** @brief Sets the gainbase linear in linear
 *
...
/** @brief Internal function for calculating the base gain value
...
/** @brief Returns the linear GAINBASE value
 *
 */
PSL_STATIC int pslGetGainbase(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Changes the on-board PARSET to the specified value
 *
 */
PSL_STATIC int pslSetParset(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/**********
 * This routine reads the DSP parameter list
 * from the board. This isn't a big deal with
 * this board since it can't change DSP code
 * dynamically.
 **********/
PSL_STATIC int pslBuildDSPList(int detChan)
{
  ...
  unsigned int lenS   = 1;
  unsigned int lenR   = 5 + RECV_BASE;
  unsigned int strLen = 0;
  unsigned int i;

  byte_t cmd       = CMD_GET_DSP_PARAM_NAMES;

  byte_t send[1];
  byte_t receive[5 + RECV_BASE];
  ...
  if (statusX != DXP_SUCCESS) {
    status = XIA_XERXES;
    sprintf(info_string,
            "Error reading number of DSP parameters for detChan %d",
            detChan);
    pslLogError("pslBuildDSPList", info_string, status);
    return status;
  }
  ...
  names = (byte_t *)utils->funcs->dxp_md_alloc((strLen + 5 + RECV_BASE) * sizeof(byte_t));

  if (names == NULL) {
    status = DXP_NOMEM;
    pslLogError("pslBuildDSPList",
                "Out-of-memory trying to create DSP list",
                status);
    return status;
  }
  ...
  if (statusX != DXP_SUCCESS) {
    status = XIA_XERXES;
    utils->funcs->dxp_md_free((void *)names);
    sprintf(info_string,
            "Error reading DSP parameter names for detChan %d",
            detChan);
    pslLogError("pslBuildDSPList", info_string, status);
    return status;
  }

  /* Create and/or clear global
   * parameter name array.
   */
  if (nParams != 0) {
    for (i = 0; i < nParams; i++) {
      utils->funcs->dxp_md_free((void *)paramNames[i]);
    }
    utils->funcs->dxp_md_free((void *)paramNames);
  }

  /* The "new" nParams */
  nParams = (unsigned int)(receive[5] | (unsigned int)receive[6] << 8);

  paramNames = (char **)utils->funcs->dxp_md_alloc(nParams * sizeof(char *));

  if (paramNames == NULL) {
    status = XIA_NOMEM;
    utils->funcs->dxp_md_free((void *)names);
    pslLogError("pslBuildDSPList",
                "Out-of-memory creating paramNames",
                status);
    return status;
  }

  for (i = 0; i < nParams; i++) {
    paramNames[i] = (char *)utils->funcs->dxp_md_alloc(MAX_DSP_PARAM_NAME_LEN * sizeof(char));
...
PSL_STATIC int pslGetParamIdx(const char *name, unsigned int *idx)
{
  ...
  status = XIA_BAD_NAME;
  sprintf(info_string,
          "%s is an unknown DSP parameter",
          name);
  pslLogError("pslGetParamIdx", info_string, status);
  return status;
...
/** @brief Sets the bytes per bin value used by the Read MCA command
 *
 */
PSL_STATIC int pslSetBytePerBin(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Set the MCA bin width multiplier
 *
 */
PSL_STATIC int pslSetBinWidth(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Get the MCA bin width multiplier
 *
 */
PSL_STATIC int pslGetBinWidth(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Get the number of MCA channels
 *
 */
PSL_STATIC int pslGetNumMCA(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the number of MCA channels
 *
 */
PSL_STATIC int pslSetNumMCA(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the baseline threshold
 *
 */
PSL_STATIC int pslGetBThresh(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the baseline threshold
 *
 */
PSL_STATIC int pslSetBThresh(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the energy threshold
 *
 */
PSL_STATIC int pslGetEThresh(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
PSL_STATIC int pslGetBaseHist(int detChan, void *value, XiaDefaults *defs)
{
   ...
   */
  for (i = 0; i < HSTLEN; i++) {
    data[i] = (short)((receive[(i * 2) + 5] |
                       ((short)receive[(i * 2) + 6] & 0xFF) << 8) & 0xFFFF);
  }
...
/**
 * @brief Asks the board for the current run status.
 *
 */
PSL_STATIC int pslGetRunActive(int detChan, void *value, XiaDefaults *defs)
{
...
/** @brief Calculates the current list of peaking times for the board
 *
 */
PSL_STATIC int pslGetCurrentPeakingTimes(int detChan, char *name,
    XiaDefaults *defs, void *value)

{
  ...
  if (status != XIA_SUCCESS) {
    sprintf(info_string,
            "Error getting number of peaking time per FiPPI "
            "from detChan %d", detChan);
    pslLogError("pslGetCurrentPeakingTimes", info_string, status);
    return status;
  }
  ...
  if (statusX != DXP_SUCCESS) {
    ...
    pslLogError("pslGetCurrentPeakingTimes", info_string, XIA_XERXES);
    return XIA_XERXES;
  }
  ...
  decIdx = RECV_DATA_OFFSET_STATUS + 2 +
           ((unsigned short)currentFiPPI * (ptPerFippi * bytePerPt + 1 ));
  DECIMATION = receive[decIdx];

  CLKSET = receive[RECV_DATA_OFFSET_STATUS];

  slowlenOffset = RECV_DATA_OFFSET_STATUS + 2 +
                  ((unsigned short)currentFiPPI * (ptPerFippi * bytePerPt + 1 )) + 1;
  ...
  pslLogDebug("pslGetCurrentPeakingTimes", info_string);
...
/** @brief Save the current values to the specified GENSET
 *
 */
PSL_STATIC int pslSaveGenset(int detChan, char *name, XiaDefaults *defs,
                             void *value)
...
/** @brief Save the current values to the specified PARSET
 *
 */
PSL_STATIC int pslSaveParset(int detChan, char *name, XiaDefaults *defs,
                             void *value)
...
/** @brief Switch to the specified FiPPI
 *
 */
PSL_STATIC int pslSetFiPPI(int detChan, char *name, XiaDefaults *defs, void *value)
{
                              ...
                              void *value)
{
  ...
  *gainMode = (unsigned short)receive[14];
 ...
 * to the user. My initial vision is that this routine would
 * be used to allocate the proper amount of memory for reading
 * out a list of peaking time ranges from Handel.
 **********/
PSL_STATIC int pslGetNumFiPPIs(int detChan, char *name, XiaDefaults *defs,
                               void *value)
{
...
/** @brief Get the peaking times and return the max and min for each FiPPi
...
/** @brief Calculate the peaking time ranges from data returned in a format
                                  ...
                                  double *ranges)
{
  ...
  for (i = 0; i < nFiPPIs; i++) {
    ...
    for (j = 0; j < ptPerFippi; j++) {
      ...
      sprintf(info_string, "val = %0f", val);
...
/** @brief Retrieves the board info (cmd 0x49)
 *
 */
 ...
 * the least significant byte. @a size is the number of bytes to use from the
 * buffer. The mantissa of a double has more bits than a long, so we'll use
 * that until we drop support of compilers without uint64_t. @a size must fit
...
/** Computes a fraction from the bits @a word. Bit 7 represents 2^-1 (0.5),
 ...
 *     0: 0.0
 *
 * @param word An unsigned value whose bits are used in the computation.
 * @param nBits Number of bits to use in the computation. Computation starts
 ...
 *
 * @param value Returned as a double * representing the temperature to 1/16 of
 * a degree Celsius.
 */
PSL_STATIC int pslGetTemperature(int detChan, char *name, XiaDefaults *defs,
                                 void *value)
...
/** @brief Converts the stored RC Tau value into microseconds
 *
 */
...
/** @brief Get the current reset interval from the hardware
 *
 */
PSL_STATIC int pslGetResetInterval(int detChan, double *value)
{
...
/** @brief Sets the preamplifier type-specific value
 *
...
/** @brief Gets the current preamplifier type-specific value
 *
 */
PSL_STATIC int pslGetPreampVal(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Set the RC Tau decay time
 *
 */
PSL_STATIC int pslSetRCTau(int detChan, XiaDefaults *defs, double *value)
{
  ...
  d = (*value) * cs;
  taurc  = (unsigned short)ROUND(d);
  *value = (double)((double)taurc / cs);
...
/** @brief Retrieves the polarity setting from the hardware
 *
 */
PSL_STATIC int pslGetPreampPol(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Lets the hardware know what the preamplifier polarity is
 *
 */
PSL_STATIC int pslSetPreampPol(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Apply changed acquisition values to the board
...
/** @brief Retrieve the current GENSET value
 *
 */
PSL_STATIC int pslGetGenset(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Find the acquisition value with the specified name
...
/** @brief Gets the gap time based on the current SLOWGAP setting
 *
 */
PSL_STATIC int pslGetEGapTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** Sets filter parameter @a n to @a value in arbitrary DSP units. This only
 *  performs the IO, not validation. @a n should come from the FILTER_*
 *  definitions, e.g. FILTER_SLOWGAP. @a value should already be
...
/** Gets filter parameter number @a n from the hardware. @a value is returned in
 * arbitrary DSP units. @a n should come from the FILTER_* definitions, e.g.
 * FILTER_SLOWGAP.
 */
PSL_STATIC int pslGetFilterParam(int detChan, byte_t n, parameter_t *value)
{
...
/** @brief Sets SLOWGAP based on the specified energy gap time
 *
 */
PSL_STATIC int pslSetEGapTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the digitizing clock speed on the board in MHz
 *
 */
PSL_STATIC int pslSetClockSpd(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the number of peaking time for each FiPPI
 *
 */
PSL_STATIC int pslGetNumPtPerFiPPI(int detChan, char *name, XiaDefaults *defs,
                                   void *value)
...
/** @brief Gets the digitizing clock speed from the board in MHz
 *
 */
PSL_STATIC int pslGetClockSpd(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the trigger peaking time from the hardware
 *
 */
PSL_STATIC int pslGetTPeakTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/**@brief Sets the trigger peaking time
 *
 */
PSL_STATIC int pslSetTPeakTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets FASTGAP based on the specified trigger gap time
 *
 */
PSL_STATIC int pslSetTGapTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the gap time based on the current FASTGAP setting
 *
 */
PSL_STATIC int pslGetTGapTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the baseline length via BLFILTER
 *
 */
PSL_STATIC int pslSetBaseLen(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the baseline filter length from BLFILTER
 *
 */
PSL_STATIC int pslGetBaseLen(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Invalidate all acquisition values of a specific type
 *
 */
PSL_STATIC int pslInvalidateAll(flag_t member, XiaDefaults *defs)
{
...
/** @brief Sets the PRESET run type and length
 *
 * @a value should be a 2 element array of doubles. The value at index 0
                            ...
                            void *value)
{
  ...
  case PRESET_LIVETIME:
    time = data[1];
    length = (unsigned long)(time / PRESET_CLOCK_TICK);
    break;

  case PRESET_OUTPUT_COUNTS:
  case PRESET_INPUT_COUNTS:
    counts = data[1];
    length = (unsigned long)counts;
    break;
  ...
  if (statusX != DXP_SUCCESS) {
    sprintf(info_string, "Error setting PRESET run: type = %#x, length = %lu",
            type, length);
...
/** @brief Retrieves the current ADC trace wait time
 *
...
/** @brief Sets the ADC tracewait time adc_trace_wait
 *
...
/** @brief Reads out all of the statistics
 *
 ...
 * @a value[0] = livetime
 * @a value[1] = realtime
 * @a value[2] = fastpeaks (input events)
 * @a value[3] = output events
 * @a value[4] = input count rate
 * @a value[5] = output count rate
 *
 */
PSL_STATIC int pslGetAllStatistics(int detChan, void *value, XiaDefaults *defs)
{
...
/** @brief Returns all of the statistics in a single array for run 
 * data module_statistics_2 @a value is expected to be a double array capable 
...
/** @brief Determines the preamplifier type based on the firmware loaded on
 * the hardware
 *
 */
PSL_STATIC int pslGetPreampType(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Querys the firmware to determine the preamplifier type
 *
 * 0 -> Reset
 * 1 -> RC Feedback
 *
...
/** @brief Get the current setting for FIPCONTROL
 *
 */
PSL_STATIC int pslGetFipControl(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the current value for RUNTASKS from the hardware
 *
 */
PSL_STATIC int pslGetRuntasks(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Set the current value for RUNTASKS on the hardware
 *
 */
PSL_STATIC int pslSetRuntasks(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the fine gain trim setting
 *
...
PSL_STATIC int pslSetGainTrim(int detChan, char *name, XiaDefaults *defs, void *value)
{
  ...
  if (gainMode < GAIN_MODE_DIGITAL) {
    ...
    GAINTWEAK = (parameter_t)(32768.0 * (*gaintrim));
  }
...
/** @brief Get the fine gain trim setting
 *
 */
PSL_STATIC int pslGetGainTrim(int detChan, char *name, XiaDefaults *defs, void *value)
{
  ...
  DEFINE_CMD(CMD_GET_GAINTWEAK, 3, 3);
  ...
  if (status != XIA_SUCCESS) {
    pslLogError("pslSetGainTrim", "Error getting gain mode.", status);
    return status;
  }
  ...
  if (statusX != DXP_SUCCESS) {
    sprintf(info_string, "Error getting GAINTWEAK for detChan %d", detChan);
    pslLogError("pslGetGainTrim", info_string, XIA_XERXES);
    return XIA_XERXES;
  }

  GAINTWEAK = (parameter_t)(((unsigned short)receive[RECV_DATA_BASE + 1] << 8) |
                            receive[RECV_DATA_BASE]);
  ...
  sprintf(info_string, "GAINTWEAK = %#x, gDB = %.3f, gaintrim = %.3f", GAINTWEAK,
          gDB, *gaintrim);
...
/** @brief Currently a stub for some TBD hardware recovery procedures
 *
 */
PSL_STATIC int pslRecover(int detChan, char *name, XiaDefaults *defs,
                          void *value)
...
/** @brief Sets the value of peak interval offset, specified in microseconds
...
/** @brief Gets the current value of peak interval offset in microseconds
...
/** @brief Sets the value of peak sample offset, specified in microseconds
...
/** @brief Gets the current value of peak sample offset in microseconds
...
/** @brief Sets the value of PEAKINT, specified in microseconds
 *
 */
PSL_STATIC int pslSetPeakInt(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the current value of PEAKINT in microseconds
 *
 */
PSL_STATIC int pslGetPeakInt(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the value of PEAKSAM, specified in microseconds
 *
 */
PSL_STATIC int pslSetPeakSam(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the current value of PEAKSAM in microseconds
 *
 */
PSL_STATIC int pslGetPeakSam(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Set the value of MAXWIDTH, specified in microseconds
 *
 */
PSL_STATIC int pslSetMaxWidth(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Get the value of MAXWIDTH in microseconds
 *
 */
PSL_STATIC int pslGetMaxWidth(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Sets the value of PEAKMODE,
...
/** @brief Gets the current value of PEAKMODE
 *
 */
PSL_STATIC int pslGetPeakMode(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Setting of BFACTOR is not supported yet
 */
PSL_STATIC int pslSetBFactor(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Gets the current value of BFACTOR
 *
 */
PSL_STATIC int pslGetBFactor(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Get the peaking_time by converting PARSET value to corresponding
 *  entry in
 */
PSL_STATIC int pslGetPeakingTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief Setting of peaking_time is not supported and must be done by
 *  setting parset.
 */
PSL_STATIC int pslSetPeakingTime(int detChan, char *name, XiaDefaults *defs, void *value)
{
...
/** @brief board operation board_features, check the DSP revision CODEREV to 
                                  ...
                                  void *value)
{
  ...
  status = pslGetParameter(detChan, "CODEREV", &CODEREV);

  if (status != XIA_SUCCESS) {
    sprintf(info_string, "Error reading CODEREV for detChan %d", detChan);
    pslLogError("pslGetBoardFeatures", info_string, status);
    return FALSE_;
  }
...
/** @brief Get sca_time_on acquisition value
 */
PSL_STATIC int pslGetScaTimeOn(int detChan, char *name, XiaDefaults *defs, void *value)
{
  int status;
  ...
  *scaon = (double)SCATIMEON;

  return XIA_SUCCESS;
}


/** @brief Set sca_time_on acquisition value
 */
PSL_STATIC int pslSetScaTimeOn(int detChan, char *name, XiaDefaults *defs, void *value)
{
  int status;
  ...
  if ((int)scaon > MAX_PULSER_PERIOD || (int)scaon < MIN_PULSER_PERIOD) {
...
/** @brief Get sca_time_off acquisition value
 */
PSL_STATIC int pslGetScaTimeOff(int detChan, char *name, XiaDefaults *defs, void *value)
{
  int status;
  ...
  *scaoff = (double)SCATIMEOFF;

  return XIA_SUCCESS;
}


/** @brief Set sca_time_off acquisition value
 */
PSL_STATIC int pslSetScaTimeOff(int detChan, char *name, XiaDefaults *defs, void *value)
{
  int status;
  ...
  if ((int)scaoff > MAX_PULSER_PERIOD || (int)scaoff < MIN_PULSER_PERIOD) {
...
/** @brief Get number_of_scas acquisition value
 */
PSL_STATIC int pslGetNumScas(int detChan, char *name, XiaDefaults *defs, void *value)
{
  int status;
  ...
  if (!isSuper) {
    sprintf(info_string, "Acquisition value number_of_scas is not supported by "
            "non-supermicro variant");
    pslLogError("pslGetNumScas", info_string, XIA_NOSUPPORT_VALUE);
    return XIA_NOSUPPORT_VALUE;
  }
   ...
   */
  if (NUMSCA > MAX_NUM_INTERNAL_SCA) {
    sprintf(info_string, "Number of SCAs is greater then the maximum allowed "
            "%d for detChan %d, resetting to default", MAX_NUM_INTERNAL_SCA, detChan);
    pslLogWarning("pslGetNumScas", info_string);

    NUMSCA = MAX_NUM_INTERNAL_SCA;
...
/** @brief Set number_of_scas acquisition value
 */
PSL_STATIC int pslSetNumScas(int detChan, char *name, XiaDefaults *defs, void *value)
{
  int status;
  ...
  if (!isSuper) {
    sprintf(info_string, "Acquisition value number_of_scas is not supported by "
            "non-supermicro variant");
    pslLogError("pslSetNumScas", info_string, XIA_NOSUPPORT_VALUE);
    return XIA_NOSUPPORT_VALUE;
  }
...
PSL_STATIC int pslGetSca(int detChan, char *name, XiaDefaults *defs, void *value)
{
  ...
  /* Use the maximum SCA number for buffer allocation
   * 1 (NUMSCA) + 4 * 4 (bytes per sca) + status
   */
  DEFINE_CMD(CMD_GET_SCALIMIT, 1, 18);

  boolean_t isSuper = dxp_is_supermicro(detChan);

  if (!isSuper) {
    sprintf(info_string, "Acquisition value sca is not supported by "
            "non-supermicro variant");
    pslLogError("pslGetSca", info_string, XIA_NOSUPPORT_VALUE);
    return XIA_NOSUPPORT_VALUE;
  }
  ...
  if (status != DXP_SUCCESS) {
    sprintf(info_string, "Error setting SCA limit %s for detChan %d", name,
            detChan);
...
PSL_STATIC int pslSetSca(int detChan, char *name, XiaDefaults *defs, void *value)
{
  ...
  /* Use the maximum SCA number for buffer allocation
   * 1 (get/set) + 1 (NUMSCA) + 4 * 4 (bytes per sca)
   * 1 (NUMSCA) + 4 * 4 (bytes per sca) + status
   */
  DEFINE_CMD(CMD_SET_SCALIMIT, 18, 18);
  ...
  if (!isSuper) {
    sprintf(info_string, "Acquisition value sca is not supported by "
            "non-supermicro variant");
    pslLogError("pslSetSca", info_string, XIA_NOSUPPORT_VALUE);
    return XIA_NOSUPPORT_VALUE;
  }
  ...
  /* Read out the SCA limits first so that we don't reset the rest of the limits */
  lenS = 1;
  lenR = 1 + 4 * (int)nSCAs + RECV_BASE + 1;
  send[0] = 0x01;
...
/** @brief Get the length of the return SCA data array sca_length, this is
...
PSL_STATIC int pslGetSCAData(int detChan, void *value, XiaDefaults *defs)
{
  ...
  pslLogError("pslGetSCAData", "Reading out of SCA data is not supported "
              "for microDXP currently.", XIA_BAD_NAME);

  return XIA_SUCCESS;
}


/** @brief Get the maximum allowed number of SCA, run data max_sca_length
*
*/
PSL_STATIC int pslGetMaxSCALength(int detChan, void *value, XiaDefaults *defs)
{
...
#ifdef XIA_ALPHA
/** @brief Perform the requested I/O operation with the tilt sensor.
 *
 * @a rw indicates the direction of the I/O: ALPHA_I2C_READ or
 * ALPHA_I2C_WRITE. @a reg refers to the registers on the tilt
 * sensor. @a data is the byte to be read or written to @a reg.
 *
...
/** @brief Initialize the tilt sensor using the configuration required
...
/** @brief Returns a 3d array of doubles via @a value corresponding to
                                     ...
                                     void *value)
{
  ...
  sprintf(info_string, "a_x = %0.6f g, a_y = %0.6f g, a_z = %0.6f g",
          accels[0], accels[1], accels[2]);
  pslLogDebug("pslUltraTiltGetOutput", info_string);

  return XIA_SUCCESS;
}


/** @brief Sets the Internal and External thresholds as defined in the
...
/** @brief Enables the DD interrupt on the tilt sensor.
 *
 * Requires UltraLo firmware and USB motherboard.
 */
PSL_STATIC int pslUltraTiltEnableInterlock(int detChan, char *name,
...
/** @brief Sets @a value as a boolean indicating if the tilt sensor has
 * triggered or not.
 *
 * Requires UltraLo firmware and USB motherboard.
 */
...
/** @brief Converts the raw byte output from the tilt sensor into a g
 * value.
 *
...
/** @brief Set the Alpha event length parameter.
 *
...
/** @brief Get the Alpha event length parameter.
 *
 *  Requires Alpha-project firmware.
 */
PSL_STATIC int pslGetAlphaEventLen(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Set the Alpha pre-buffer length parameter.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslSetAlphaPreBufferLen(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Get the Alpha pre-buffer length parameter.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslGetAlphaPreBufferLen(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Set the Alpha DAC target parameter.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslSetAlphaDACTarget(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Get the Alpha DAC target parameter.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslGetAlphaDACTarget(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Set the Alpha DAC tolerance parameter.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslSetAlphaDACTolerance(int detChan, char *name, XiaDefaults *defs,
...
/** @brief Get the Alpha DAC tolerance parameter.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslGetAlphaDACTolerance(int detChan, char *name, XiaDefaults *defs,
...
/* @brief Set an Alpha parameter by index.
 *
 * Requires Alpha-project firmware.
 */
PSL_STATIC int pslSetAlphaParam(int detChan, unsigned int idx,
...
/** @brief Get an Alpha parameter by index.
 *
 ...
 * @a buf. (It should be large enough to hold @a nEvt number of event blocks.)
 */
PSL_STATIC int pslAlphaReadFromEventBuffer(int detChan, unsigned short startIdx,
    unsigned short nEvt,
    unsigned short *buf)
...
/** @brief Returns the results of running the Status command
 *
 */
PSL_STATIC int pslGetHardwareStatus(int detChan, char *name, XiaDefaults *defs,
                                    void *value)
...
#ifdef XIA_ALPHA
/** @brief Enables or disables the external trigger.
 *
 ...
 * Requires Alpha-project firmware.
 *
 * @param value Returns high voltage value as an unsigned short.
 */
PSL_STATIC int pslGetAlphaHV(int detChan, char *name, XiaDefaults *defs,
                             void *value)
{
 ...
 * Requires Alpha-project firmware.
 *
 * @param value unsigned short representing the target voltage.
 */
PSL_STATIC int pslSetAlphaHV(int detChan, char *name, XiaDefaults *defs,
                             void *value)
{
 ...
 * Requires Alpha-project firmware.
 *
 * @param value Returns CPLD firmware version a unsigned long value
 */
PSL_STATIC int pslGetCPLDVersion(int detChan, char *name, XiaDefaults *defs,
                                 void *value)
{
...
/**
 * Configure Pulser 1. Expects a 3-tuple of unsigned shorts in @a value:
...
/**
 * Configure Pulser 2. Expects a 4-tuple of unsigned shorts in @a value:
...
/**
 * Sets the pulser mode. @a value allows the bit masks specified
 * in psl_udxp_alpha.h.
 */
PSL_STATIC int pslAlphaPulserSetMode(int detChan, char *name, XiaDefaults *defs,
                                     void *value)
...
/**
 * Configures the veto pulse. Accepts a tuple of bytes in @a value:
                                     ...
                                     XiaDefaults *defs)
{
  ...
  stats[0] = LIVETIME_CLOCK_TICK * ((double)receive[5] +
                                    ldexp((double)receive[6], 8) +
                                    ldexp((double)receive[7], 16) +
                                    ldexp((double)receive[8], 24) +
                                    ldexp((double)receive[9], 32) +
                                    ldexp((double)receive[10], 40));

  stats[1] = ALPHA_REALTIME_CLOCK_TICK * ((double)receive[11] +
                                          ldexp((double)receive[12], 8) +
                                          ldexp((double)receive[13], 16) +
                                          ldexp((double)receive[14], 24) +
                                          ldexp((double)receive[15], 32) +
                                          ldexp((double)receive[16], 40));

  return XIA_SUCCESS;
}

/** Sets the module referenced by @a detChan as a realtime clock
 *  master.
 */
PSL_STATIC int pslUltraSetAsClockMaster(int detChan, char *name,
                                        XiaDefaults *defs, void *value)
 ...
 * in the I2C EEPROM.
 *
 * @param value Returns the unique ID as a double. The value derives from a
...
/** Computes a DOW CRC over @a buffer according to the lookup table algorithm
...
#ifndef EXCLUDE_USB2
/** Reads the uDXP CPLD firmware version number (2 bytes each)
 *
 ...
 * Requires Alpha or SuperMicro firmware.
 *
 * @param value Returns CPLD firmware variant a unsigned long value
 */
PSL_STATIC int pslGetUdxpCPLDVariant(int detChan, char *name, XiaDefaults *defs,
                                     void *value)
{
...
/** Reads the uDXP USB firmware version number (2 bytes)
 *
 * Requires Alpha or SuperMicro firmware.
 *
 * @param value Returns USB firmware version as a unsigned long
 */
PSL_STATIC int pslGetUSBVersion(int detChan, char *name, XiaDefaults *defs,
                                void *value)
{
  ...
  /* Reverse the bytes in [3]Major [2]Minor [1-0]Revision as in
     other versions */
...
/** @brief Set the path used when the XUP module writes out backup files
 *
 */
PSL_STATIC int pslSetXUPBackupPath(int detChan, char *name, XiaDefaults *defs,
                                   void *value)
...
/** @brief Creates a master parameter set for use with other boards
 *
 */
PSL_STATIC int pslCreateMasterParamSet(int detChan, char *name,
                                       XiaDefaults *defs, void *value)
...
/** @brief Creates a backup of the microDXP memory.
 *
