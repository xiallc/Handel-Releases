...
/** @file xia_crc.h
 * @brief Functions related to calculating CRC32 hashes for various sources.
 */

#ifndef XIA_UTIL_CRC_H
#define XIA_UTIL_CRC_H
 ...
 */
static uint32_t xia_crc32(uint32_t crc, const unsigned char* data, int len) {
...
/**
 * @brief Calculates the CRC32 of a stream.
 * @param[in] stream: A stream that can be used to calculate the CRC32.
 * @returns The CRC32 of the file stream or 0 if there was an error.
 */
static uint32_t xia_crc32_stream(FILE* stream) {
    if (stream == NULL) {
        return 0;
    }
    ...
    while (ch != EOF) {
        crc = xia_crc32(crc, (unsigned char*) &ch, 1);
        ch = fgetc(stream);
    }
...
/**
 * @brief Opens a file and calculates the CRC
 * @param[in] filename: The file's name.
 * @return The CRC32 of the provided file.
 */
static uint32_t xia_crc32_file(char* filename) {
    if (filename == NULL) {
        return 0;
    }
    ...
    uint32_t crc = xia_crc32_stream(fp);

    fclose(fp);
    return crc;
}

#endif //XIA_UTIL_CRC_H